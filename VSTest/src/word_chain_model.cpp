...
void WordChainModel::init() {
    for (int i = 1; i <= ALPHA_COUNT; i++) {
        graph.add_edge(WORD_START_NODE, WORD_START_ALPHA_NODE(i), 0);
        graph.add_edge(WORD_END_ALPHA_NODE(i), WORD_END_NODE, 0);
    }
    auto length = words.size();
    for (int i = 0; i < length; i++) {
        int word_index = i + 1;
        int word_node_index = WORD_WORD_NODE(word_index);
        std::string word = words[i];

        char prefix_char = word.at(0);
        int prefix_char_index = char_to_index(prefix_char);
        int prefix_char_node_index = WORD_START_ALPHA_NODE(prefix_char_index);
        ...
        int suffix_char_node_index = WORD_END_ALPHA_NODE(suffix_char_index);
    ...
    for (int i = 0; i < length; i++) {
        for (int j = 0; j < length; j++) {
            ...
            int word_node_index_i = WORD_WORD_NODE(word_index_i);
            std::string word_i = words[i];

            int word_index_j = j + 1;
            int word_node_index_j = WORD_WORD_NODE(word_index_j);
...
std::vector<std::string> *WordChainModel::get_longest_chain(char start_char, char end_char) {
    int start_node = (start_char == NO_START_CHAR) ? WORD_START_NODE : WORD_START_ALPHA_NODE(char_to_index(start_char));
    int end_node = (end_char == NO_END_CHAR) ? WORD_END_NODE : WORD_END_ALPHA_NODE(char_to_index(end_char));
    ...
    if (node_link == nullptr) {
        ...
        for (const int &node_index : node_list) {
            if (WORD_IS_WORD_NODE(node_index)) {
                int word_index = WORD_WORD_NODE_TO_INDEX(node_index);
